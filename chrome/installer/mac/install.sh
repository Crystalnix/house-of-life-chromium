...
# usage: install.sh update_dmg_mount_point application_bundle_path
#
...
usage() {
  echo "usage: ${ME} update_dmg_mount_point application_bundle_path" >& 2
}

main() {
  local update_dmg_mount_point="${1}"
  local application_bundle_path="${2}"
  ...
  local update_version_app product_id
  if [[ -z "${is_patch}" ]]; then
...
"${patch_dir}/version_${update_version_app_old}_${update_version_app}.dirpatch"
  ...
  local installed_app
  if ! installed_app="${application_bundle_path}" ||
...
# FIXME: not sure if we can just omit the following
